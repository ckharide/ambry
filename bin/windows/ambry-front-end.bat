...
IF ["%AMBRY_SERVER_PROPS%"] EQU [""] (
    set AMBRY_SERVER_PROPS=--serverPropsFilePath=%BASE_DIR%\config\frontend.properties
)
...
IF ["%CLASSPATH%%"] EQU [""] (
    set CLASSPATH=-cp %BASE_DIR%\target\ambry.jar
)
...
set COMMAND=%JAVA% %AMBRY_LOG4J_OPTS% %CLASSPATH% com.github.ambry.frontend.AmbryFrontendMain  %AMBRY_SERVER_PROPS% %AMBRY_HARDWARE_PROPS% %AMBRY_PART_PROPS%  
rem echo.
rem echo %COMMAND%
rem echo.
